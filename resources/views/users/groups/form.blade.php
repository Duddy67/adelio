...
@section ('main')
    ...
    @php $action = (isset($group)) ? route('users.groups.update', $query) : route('users.groups.store', $query) @endphp
    <form method="post" action="{{ $action }}" id="itemForm">
        ...
        <input type="hidden" id="cancelEdit" value="{{ route('users.groups.cancel', $query) }}">
        <input type="hidden" id="close" name="_close" value="0">
    </form>
    <x-toolbar :items=$actions />

    @if (isset($group))
        <form id="deleteItem" action="{{ route('users.groups.destroy', $query) }}" method="post">
            @method('delete')
            @csrf
        </form>
    @endif
