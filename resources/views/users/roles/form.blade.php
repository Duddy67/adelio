...
@section ('main')
    ...
    @php $action = (isset($role)) ? route('users.roles.update', $query) : route('users.roles.store', $query) @endphp
	...
	<input type="hidden" id="cancelEdit" value="{{ route('users.roles.cancel', $query) }}">
    ...
    @if (isset($role))
	<form id="deleteItem" action="{{ route('users.roles.destroy', $query) }}" method="post">
	    @method('delete')
	    @csrf
	</form>
    @endif
