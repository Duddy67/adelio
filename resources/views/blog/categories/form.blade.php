...
@section ('main')
    ...
    @php $action = (isset($category)) ? route('blog.categories.update', $query) : route('blog.categories.store', $query) @endphp
    <form method="post" action="{{ $action }}" id="itemForm">
        ...
        <input type="hidden" id="cancelEdit" value="{{ route('blog.categories.cancel', $query) }}">
    ...
    @if (isset($category))
        <form id="deleteItem" action="{{ route('blog.categories.destroy', $query) }}" method="post">
            @method('delete')
            @csrf
        </form>
    @endif
