...
@section ('main')
    ...
    @php $action = (isset($post)) ? route('blog.posts.update', $query) : route('blog.posts.store', $query) @endphp
    <form method="post" action="{{ $action }}" id="itemForm" enctype="multipart/form-data">
        ...
        <input type="hidden" id="cancelEdit" value="{{ route('blog.posts.cancel', $query) }}">
    ...
    @if (isset($post))
        <form id="deleteItem" action="{{ route('blog.posts.destroy', $query) }}" method="post">
            @method('delete')
            @csrf
        </form>
    @endif
