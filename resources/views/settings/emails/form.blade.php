...
@section ('main')
    ...
    @php $action = (isset($email)) ? route('settings.emails.update', $query) : route('settings.emails.store', $query) @endphp
    <form method="post" action="{{ $action }}" id="itemForm">
        ...
        <input type="hidden" id="cancelEdit" value="{{ route('settings.emails.cancel', $query) }}">
        <input type="hidden" id="close" name="_close" value="0">
    </form>
    <x-toolbar :items=$actions />

    @if (isset($email))
	<form id="deleteItem" action="{{ route('settings.emails.destroy', $query) }}" method="post">
	    @method('delete')
	    @csrf
	</form>
    @endif
