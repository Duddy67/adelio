...
class EmailController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('settings.emails');
    ...
    public function index(Request $request)
    {
        ...
        $query = $request->query();
        $url = ['route' => 'settings.emails', 'item_name' => 'email', 'query' => $query];

        return view('settings.emails.list', compact('items', 'columns', 'rows', 'actions', 'filters', 'url', 'query'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('settings.emails.form', compact('fields', 'actions', 'query'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($email->checked_out && $email->checked_out != auth()->user()->id) {
            return redirect()->route('settings.emails.index')->with('error',  __('messages.generic.checked_out'));
        }
        ...
        return view('settings.emails.form', compact('email', 'fields', 'actions', 'query'));
    }
    ...
    public function cancel(Request $request, Email $email = null)
    {
        ...
        return redirect()->route('settings.emails.index', $request->query());
    }
    ...
    public function massCheckIn(Request $request)
    {
        ...
        return redirect()->route('settings.emails.index', $request->query())->with($messages);
    }
    ...
    public function update(UpdateRequest $request, Email $email)
    {
        ...
        if ($request->input('_close', null)) {
            $email->checkIn();
            return redirect()->route('settings.emails.index', $request->query())->with('success', __('messages.emails.update_success'));
        }

        return redirect()->route('settings.emails.edit', array_merge($request->query(), ['email' => $email->id]))->with('success', __('messages.emails.update_success'));
    }
    ...
    public function store(StoreRequest $request)
    {
        ...
        if ($request->input('_close', null)) {
            return redirect()->route('settings.emails.index', $request->query())->with('success', __('messages.emails.create_success'));
        }

        return redirect()->route('settings.emails.edit', array_merge($request->query(), ['email' => $email->id]))->with('success', __('messages.emails.create_success'));
    }
    ...
    public function destroy(Request $request, Email $email)
    {
        ...
        return redirect()->route('settings.emails.index', $request->query())->with('success', __('messages.emails.delete_success', ['name' => $code]));
    }
