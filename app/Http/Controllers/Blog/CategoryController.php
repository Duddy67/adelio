...
class CategoryController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('blog.categories');
    ...
    public function index(Request $request)
    {
        ...
        $query = $request->query();
        $url = ['route' => 'blog.categories', 'item_name' => 'category', 'query' => $query];

        return view('blog.categories.list', compact('items', 'columns', 'rows', 'actions', 'filters', 'url', 'query'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('blog.categories.form', compact('fields', 'actions', 'tab', 'query'));
    }
    ...
    public function edit(Request $request, $id, $tab = null)
    {
        ...
        if (!$category->canAccess()) {
            return redirect()->route('blog.categories.index')->with('error',  __('messages.generic.access_not_auth'));
        }

        if ($category->checked_out && $category->checked_out != auth()->user()->id) {
            return redirect()->route('blog.categories.index')->with('error',  __('messages.generic.checked_out'));
        }
        ...
        return view('blog.categories.form', compact('category', 'fields', 'actions', 'tab', 'query'));
    }
    ...
    public function cancel(Request $request, Category $category = null)
    {
        ...
        return redirect()->route('blog.categories.index', $request->query());
    }
    ...
    public function update(UpdateRequest $request, Category $category)
    {
        if ($category->checked_out != auth()->user()->id) {
            return redirect()->route('blog.categories.index', $request->query())->with('error',  __('messages.generic.user_id_does_not_match'));
        }

        if (!$category->canEdit()) {
            return redirect()->route('blog.categories.index', $request->query())->with('error',  __('messages.generic.edit_not_auth'));
        }
        ...
        if ($request->input('parent_id')) {
            ...
            if ($category->id == $request->input('parent_id') || $parent->isDescendantOf($category)) {
                return redirect()->route('blog.categories.edit', array_merge($request->query(), ['category' => $category->id]))->with('error',  __('messages.generic.must_not_be_descendant'));
            }

            if ($parent->access_level == 'private' && $parent->owned_by != auth()->user()->id) {
                return redirect()->route('blog.categories.create', $request->query())->with('error',  __('messages.generic.item_is_private', ['name' => $parent->name]));
            }
        }
        ...
        if ($category->canChangeAccessLevel()) {
            ...
            if ($category->access_level != 'private') {
                ...
                if ($request->input('access_level') == 'private' && !$category->canDescendantsBePrivate()) {
                    return redirect()->route('blog.categories.edit', array_merge($request->query(), ['category' => $category->id]))->with('error',  __('messages.generic.descendants_cannot_be_private'));
                }

                if ($request->input('access_level') == 'private' && $category->anyDescendantCheckedOut()) {
                    return redirect()->route('blog.categories.edit', array_merge($request->query(), ['category' => $category->id]))->with('error',  __('messages.generic.descendants_checked_out'));
                }
        ...
        if ($request->input('_close', null)) {
            ...
            return redirect()->route('blog.categories.index', $request->query())->with('success', __('messages.categories.update_success'));
        }

        return redirect()->route('blog.categories.edit', array_merge($request->query(), ['category' => $category->id, 'tab' => $request->input('_tab')]))->with('success', __('messages.categories.update_success'));
    }
    ...
    public function store(StoreRequest $request)
    {
        ...
        if ($request->input('parent_id')) {
            ...
            if ($parent->access_level == 'private' && $request->input('access_level') != 'private') {
                return redirect()->route('blog.categories.create', $request->query())->with('error',  __('messages.generic.access_level_must_be_private'));
            }

            if ($parent->access_level == 'private' && $request->input('owned_by') != $parent->owned_by) {
                return redirect()->route('blog.categories.create', $request->query())->with('error',  __('messages.generic.owner_must_match_parent_category'));
            }
        }
        ...
        if ($request->input('_close', null)) {
            return redirect()->route('blog.categories.index', $request->query())->with('success', __('messages.categories.create_success'));
        }

        return redirect()->route('blog.categories.edit', array_merge($request->query(), ['category' => $category->id]))->with('success', __('messages.categories.create_success'));
    }
    ...
    public function destroy(Request $request, Category $category)
    {
        if (!$category->canDelete() || !$category->canDeleteDescendants()) {
            return redirect()->route('blog.categories.edit', array_merge($request->query(), ['category' => $category->id]))->with('error',  __('messages.generic.delete_not_auth'));
        }
        ...
        return redirect()->route('blog.categories.index', $request->query())->with('success', __('messages.categories.delete_success', ['name' => $name]));
    }
    ...
    public function massDestroy(Request $request)
    {
        ...
        foreach ($request->input('ids') as $id) {
            ...
            if (!$category->canDelete() || !$category->canDeleteDescendants()) {
                return redirect()->route('blog.categories.index', $request->query())->with(
                [
        ...
        return redirect()->route('blog.categories.index', $request->query())->with('success', __('messages.categories.delete_list_success', ['number' => $deleted]));
    }
    ...
    public function massCheckIn(Request $request)
    {
        ...
        return redirect()->route('blog.categories.index', $request->query())->with($messages);
    }
    ...
    public function massPublish(Request $request)
    {
        ...
        return redirect()->route('blog.categories.index', $request->query())->with('success', __('messages.categories.change_status_list_success', ['number' => $changed]));
    }
