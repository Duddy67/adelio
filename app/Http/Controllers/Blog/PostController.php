...
class PostController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = $request->query();
        $url = ['route' => 'blog.posts', 'item_name' => 'post', 'query' => $query];

        return view('blog.posts.list', compact('items', 'columns', 'rows', 'actions', 'filters', 'url', 'query'));
    }
    ...
    public function create(Request $request)
    {
        ...
        return view('blog.posts.form', compact('fields', 'actions', 'tab',  'query'));
    }
    ...
    public function edit(Request $request, $id, $tab = null)
    {
        ...
        if (!$post->canAccess()) {
            return redirect()->route('blog.posts.index')->with('error',  __('messages.generic.access_not_auth'));
        }

        if ($post->checked_out && $post->checked_out != auth()->user()->id) {
            return redirect()->route('blog.posts.index')->with('error',  __('messages.generic.checked_out'));
        }
        ...
        return view('blog.posts.form', compact('post', 'fields', 'actions', 'tab', 'query'));
    }
    ...
    public function cancel(Request $request, Post $post = null)
    {
        ...
        return redirect()->route('blog.posts.index', $request->query());
    }
    ...
    public function update(UpdateRequest $request, Post $post)
    {
        if ($post->checked_out != auth()->user()->id) {
            return redirect()->route('blog.posts.index', $request->query())->with('error',  __('messages.generic.user_id_does_not_match'));
        }

        if (!$post->canEdit()) {
            return redirect()->route('blog.posts.edit', array_merge($request->query(), ['post' => $post->id]))->with('error',  __('messages.generic.edit_not_auth'));
        }
        ...
        if ($request->input('_close', null)) {
            ...
            return redirect()->route('blog.posts.index', $request->query())->with('success', __('messages.posts.update_success'));
        }

        return redirect()->route('blog.posts.edit', array_merge($request->query(), ['post' => $post->id, 'tab' => $request->input('_tab')]))->with('success', __('messages.posts.update_success'));
    }
    ...
    public function store(StoreRequest $request)
    {
        ...
        if ($request->input('_close', null)) {
            return redirect()->route('blog.posts.index', $request->query())->with('success', __('messages.posts.create_success'));
        }

        return redirect()->route('blog.posts.edit', array_merge($request->query(), ['post' => $post->id]))->with('success', __('messages.posts.create_success'));
    }
    ...
    {
        if (!$post->canDelete()) {
            return redirect()->route('blog.posts.edit', array_merge($request->query(), ['post' => $post->id]))->with('error',  __('messages.generic.delete_not_auth'));
        }

        $name = $post->name;
        $post->delete();

        return redirect()->route('blog.posts.index', $request->query())->with('success', __('messages.posts.delete_success', ['name' => $name]));
    }
    ...
    public function massDestroy(Request $request)
    {
        ...
        foreach ($request->input('ids') as $id) {
            ...
            if (!$post->canDelete()) {
                return redirect()->route('blog.posts.index', $request->query())->with(
                [
        ...
        return redirect()->route('blog.posts.index', $request->query())->with('success', __('messages.posts.delete_list_success', ['number' => $deleted]));
    }
    ...
    public function massCheckIn(Request $request)
    {
        ...
        return redirect()->route('blog.posts.index', $request->query())->with($messages);
    }
    ...
    public function massUpdate(Request $request)
    {
        ...
        return redirect()->route('blog.posts.index')->with($messages);
    }
    ...
    public function massPublish(Request $request)
    {
        ...
        return redirect()->route('blog.posts.index', $request->query())->with('success', __('messages.posts.publish_list_success', ['number' => $published]));
    }
    ...
    public function massUnpublish(Request $request)
    {
        ...
        return redirect()->route('blog.posts.index', $request->query())->with('success', __('messages.posts.unpublish_list_success', ['number' => $unpublished]));
    }
